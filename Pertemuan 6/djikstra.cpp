...
void dijkstra(int V, vector<vector<iPair>> &adj, int src) {
    ...
    pq.push({0, src}); //masukkan sumber ke dalam priority queue

    //loop djikstra
    while (!pq.empty()) {
        int d = pq.top().first; //ambil jarak terpendek
        int u = pq.top().second; //ambil vertex dengan jarak terpendek
        pq.pop();

        //jika jarak yang diambil lebih besar dari jarak yang sudah disimpan, lewati
        if (d > dist[u]) continue;

        //periksa semua tetangga dari simpul u
        for (auto edge : adj[u]) {
            int v = edge.first; // vertex tetangga
            int w = edge.second; //bobot dei u -> v


            //update jarak jika ditemukan jarak yang lebih pendek, update jarak ke v
            if (dist[u] + w < dist[v]) {
                dist[v] = dist[u] + w;
                pq.push({dist[v], v});
            }
